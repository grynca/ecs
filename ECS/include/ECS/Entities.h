...
namespace Grynca
{
	...
	public:
		...
		// 	Entity* is not for persistent storage
		//	store entity->guid() instead
		...
		// get entity's component based on dynamic component family id
		uint8_t* getComponent(Entity* entity, unsigned int component_family_id);

		// get entity's component with static component type
...
inline Grynca::Entities::Entities(ComponentsRegister& comps_reg)
...
template <typename EntType>
inline void Grynca::Entities::registerEntityType(unsigned int initial_capacity)
{
	...
	_entity_pools.back().initial_update_mask = EntType::initialUpdateMask();
...
inline unsigned int Grynca::Entities::registeredEntitiesCount()
{
	return _entity_pools.size();
}

inline const Grynca::ComponentsMask& Grynca::Entities::componentsMask(unsigned int entity_type_id)
{
	assert(entity_type_id < _entity_pools.size());
	return _entity_pools[entity_type_id].components_mask;
}

inline const Grynca::ComponentsMask& Grynca::Entities::initialUpdateMask(unsigned int entity_type_id)
{
	assert(entity_type_id < _entity_pools.size());
	return _entity_pools[entity_type_id].initial_update_mask;
}

inline Grynca::Entity* Grynca::Entities::addEntity(unsigned int entity_type_id)
{
	assert(entity_type_id < _entity_pools.size());
...
inline void Grynca::Entities::deleteEntity(Entity* entity)
{
	assert(entity);
	assert(entity->_type_id < _entity_pools.size());
	assert(entity->_local_id < _entity_pools[entity->typeId()].entities.size() );
...
inline Grynca::Entity* Grynca::Entities::getEntity(unsigned int guid)
{
	std::map<uint32_t, EntityPos>::iterator it = _entities_pos_map.find(guid);
...
inline uint8_t* Grynca::Entities::getComponent(Entity* entity, unsigned int component_family_id)
{
	assert(entity);
	assert(entity->_type_id < _entity_pools.size());
...
template <typename CompType>
inline CompType* Grynca::Entities::getComponent(Entity* entity)
{
